...
func (p *Post) SavePost(db *gorm.DB) (*Post, error) {
	...
	if p.ID != 0 {
		err = db.Debug().Model(&SysStf{}).Where("id = ?", p.AuthorID).Take(&p.Author).Error
		if err != nil {
			return &Post{}, err
		}
	}
...
func (p *Post) FindAllPosts(db *gorm.DB) (*[]Post, error) {
	...
	if len(posts) > 0 {
		for i, _ := range posts {
			err := db.Debug().Model(&SysStf{}).Where("id = ?", posts[i].AuthorID).Take(&posts[i].Author).Error
			if err != nil {
				return &[]Post{}, err
			}
		}
