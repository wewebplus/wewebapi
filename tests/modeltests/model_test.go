...
func refreshUserTable() error {
	...
	err := server.DB.Debug().DropTableIfExists(&models.SysStf{}).Error
	if err != nil {
		return err
	}
	server.DB.Exec("SET foreign_key_checks=1")
	err = server.DB.Debug().AutoMigrate(&models.SysStf{}).Error
	if err != nil {
		return err
	}
...
func seedOneUser() (models.SysStf, error) {
	...
	err := server.DB.Debug().Model(&models.SysStf{}).Create(&user).Error
	if err != nil {
...
func seedUsers() error {
	...
	for i := range users {
		err := server.DB.Debug().Model(&models.SysStf{}).Create(&users[i]).Error
		if err != nil {
			return err
		}
	}
...
func refreshUserAndPostTable() error {
	...
	err := server.DB.Debug().DropTableIfExists(&models.Post{}, &models.SysStf{}).Error
	if err != nil {
		return err
	}
	server.DB.Exec("SET foreign_key_checks=1")
	err = server.DB.Debug().AutoMigrate(&models.SysStf{}, &models.Post{}).Error
	if err != nil {
		return err
	}
