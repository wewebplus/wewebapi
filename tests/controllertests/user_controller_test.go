...
func TestGetUserByID(t *testing.T) {
	...
	userSample := []struct {
		...
		{
			id:         strconv.Itoa(int(user.SyStfId)),
			statusCode: 200,
	...
	for _, v := range userSample {
		...
		if v.statusCode == 200 {
			assert.Equal(t, user.SyStfFnameeng, responseMap["nickname"])
			assert.Equal(t, user.SyStfEmail, responseMap["email"])
		}
	}
}
